...
for (j in 1:length(busco_files)) {
  ...
  busco_file_path <- file.path(directory, busco_files[1])
  chrom_file_path <- file.path(directory, chrom_files[1])
...
ggsave("removed.png", plot = grid.arrange(grobs = plot_list, ncol = 1), width = 10, height = 20)
################################################################################